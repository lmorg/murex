...
func (p *Parameters) ParseFlags(args *Arguments) (flags map[string]string, additional []string, err error) {
	...
	for i := range p.Params {
		switch {
		case strings.HasPrefix(p.Params[i], "-"):
			switch {
			case previous != "":
				return nil, nil, errors.New("Invalid parameters! Flag found without value: " + previous)
			case args.Flags[p.Params[i]] == types.Boolean:
				flags[p.Params[i]] = types.TrueString
			case args.Flags[p.Params[i]] != "":
				previous = p.Params[i]
			default:
				return nil, nil, errors.New("Invalid parameters! Flag not recognised: " + p.Params[i])
			}

		case previous != "":
			flags[previous] = p.Params[i]
			previous = ""
		...
		default:
			...
			additional = append(additional, p.Params[i])
		}
	}
