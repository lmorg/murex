...
func File(_ context.Context, _ []rune, filename string, incImages bool, size *readline.PreviewSizeT) ([]string, int, error) {
...
func Command(_ context.Context, _ []rune, command string, _ bool, size *readline.PreviewSizeT) ([]string, int, error) {
	if lang.GlobalAliases.Exists(command) {
		alias := lang.GlobalAliases.Get(command)
		if len(alias) == 0 {
			return nil, 0, nil
		}
		return Command(nil, nil, alias[0], false, size)
	}
...
func Parameter(_ context.Context, block []rune, parameter string, incImages bool, size *readline.PreviewSizeT) ([]string, int, error) {
	parameter = variables.ExpandString(parameter)
	if utils.Exists(parameter) {
		return File(nil, nil, parameter, incImages, size)
	}

	pt, _ := parser.Parse(block, 0)
	lines, _, err := Command(nil, nil, pt.FuncName, false, size)
	if err != nil {
		return lines, 0, err
	}
	for i := range lines {
