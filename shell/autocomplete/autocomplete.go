...
func MatchFunction(partial string, errCallback func(error)) (items []string) {
	switch {
...
func MatchFlags(flags []Flags, partial, exe string, params []string, pIndex *int, defs *map[string]string, tdt *readline.TabDisplayType, errCallback func(error)) (items []string) {
	args := dynamicArgs{
		exe:    exe,
		params: params,
	}

	return matchFlags(flags, partial, exe, params, pIndex, args, defs, tdt, errCallback)
}
