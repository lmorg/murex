...
* `--jmap`
    Write a `json` map to stdout instead of an array. `--jmap` be used with `--step` nor `--parallel`
* `--parallel`
...
* `--step`
    `<int>` Iterates in steps. Value passed to block is an array of items in the step range. However step does not run functions in parallel; use `--parallel` if you need concurrent execution. `--step` be used with `--jmap` nor `--parallel`
