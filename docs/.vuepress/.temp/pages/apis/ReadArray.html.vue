<template><div><h1 id="readarray-type" tabindex="-1"><a class="header-anchor" href="#readarray-type" aria-hidden="true">#</a> <code v-pre>ReadArray()</code> (type)</h1>
<blockquote>
<p>Read from a data type one array element at a time</p>
</blockquote>
...
<p>Registering your <code v-pre>ReadArray()</code></p>
...
stdio<span class="token punctuation">.</span><span class="token function">RegisterReadArray</span><span class="token punctuation">(</span><span class="token comment">/* your type name */</span><span class="token punctuation">,</span> <span class="token comment">/* your readArray func */</span><span class="token punctuation">)</span>
...
<p>Example <code v-pre>ReadArray()</code> function:</p>
...
<span class="token keyword">func</span> <span class="token function">readArray</span><span class="token punctuation">(</span>ctx context<span class="token punctuation">.</span>Context<span class="token punctuation">,</span> read stdio<span class="token punctuation">.</span>Io<span class="token punctuation">,</span> callback <span class="token keyword">func</span><span class="token punctuation">(</span><span class="token punctuation">[</span><span class="token punctuation">]</span><span class="token builtin">byte</span><span class="token punctuation">)</span><span class="token punctuation">)</span> <span class="token builtin">error</span> <span class="token punctuation">{</span>
	...
	<span class="token keyword">for</span> scanner<span class="token punctuation">.</span><span class="token function">Scan</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
		...
		<span class="token keyword">default</span><span class="token punctuation">:</span>
			<span class="token function">callback</span><span class="token punctuation">(</span>bytes<span class="token punctuation">.</span><span class="token function">TrimSpace</span><span class="token punctuation">(</span>scanner<span class="token punctuation">.</span><span class="token function">Bytes</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">)</span>
		<span class="token punctuation">}</span>
	<span class="token punctuation">}</span>
...
you pass your array to <code v-pre>lang.ArrayTemplate()</code> which is a handler to convert Go
structures into Murex arrays. This also makes writing <code v-pre>ReadArray()</code> handlers
easier since you can just pass <code v-pre>lang.ArrayTemplate()</code> your marshaller.
For example:</p>
...
<li><code v-pre>func([]byte)</code>: callback function. Each callback will be a []byte slice containing an array element</li>
</ol>
