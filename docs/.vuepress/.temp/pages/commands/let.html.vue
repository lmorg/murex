<template><div><h1 id="let" tabindex="-1"><a class="header-anchor" href="#let" aria-hidden="true">#</a> <code v-pre>let</code></h1>
<blockquote>
<p>Evaluate a mathematical function and assign to variable (deprecated)</p>
</blockquote>
<h2 id="description" tabindex="-1"><a class="header-anchor" href="#description" aria-hidden="true">#</a> Description</h2>
<p><code v-pre>let</code> evaluates a mathematical function and then assigns it to a locally
scoped variable (like <code v-pre>set</code>)</p>
...
<pre><code>let var_name=evaluation
...
<h3 id="other-operators" tabindex="-1"><a class="header-anchor" href="#other-operators" aria-hidden="true">#</a> Other Operators</h3>
<p><code v-pre>let</code> also supports the following operators (substitute <strong>VAR</strong> with your
variable name, and <strong>NUM</strong> with a number):</p>
<ul>
<li><code v-pre>VAR--</code>, subtract 1 from VAR</li>
<li><code v-pre>VAR++</code>, add 1 to VAR</li>
<li><code v-pre>VAR -= NUM</code>, subtract NUM from VAR</li>
<li><code v-pre>VAR += NUM</code>, add NUM to VAR</li>
<li><code v-pre>VAR /= NUM</code>, divide VAR by NUM</li>
<li><code v-pre>VAR *= NUM</code>, multiply VAR by NUM</li>
</ul>
<p>eg</p>
<pre><code>» let: i=0
» let: i++
...
<p>Please note these operators are not supported by <code v-pre>=</code>.</p>
...
<pre><code>» set abc=123
» = $abc==123
true
</code></pre>
<p>Directly by name:</p>
<pre><code>» set abc=123
» = abc==123
false
</code></pre>
<p>To understand the difference between the two, you must first understand how
string interpolation works; which is where the parser tokenised the parameters
like so</p>
<pre><code>command line: = $abc==123
...
<p>Then when the command line gets executed, the parameters are compiled on demand
similarly to this crude pseudo-code</p>
<pre><code>command: &quot;=&quot;
parameters 1: concatenate(&quot;&quot;, GetValue(abc), &quot;==123&quot;)
output: &quot;=&quot; &quot;123==123&quot;
</code></pre>
<p>Thus the actual command getting run is literally <code v-pre>123==123</code> due to the variable
...
<p>The main advantage (or disadvantage, depending on your perspective) of using
variables this way is that their data-type is preserved.</p>
<pre><code>» set str abc=123
» = abc==123
false
...
<p>Unfortunately is one of the biggest areas in Murex where you'd need to be
...
<p>Please be mindful that if you use string interpolation then you will need to
instruct <code v-pre>=</code> and <code v-pre>let</code> that your field is a string</p>
<pre><code>» set str abc=123
» = `$abc`==`123`
true
</code></pre>
<h3 id="best-practice-recommendation" tabindex="-1"><a class="header-anchor" href="#best-practice-recommendation" aria-hidden="true">#</a> Best practice recommendation</h3>
...
<h3 id="read-more" tabindex="-1"><a class="header-anchor" href="#read-more" aria-hidden="true">#</a> Read more</h3>
