...
function that was part of the shell itself. For this reason, $SHELLs based on
that lineage, be it the Bourne shell, Bash or Zsh, all share a scripting syntax
which very much feels like it is extended from REPL usage.
...
a little. So I decided to make the quotation marks optional. Then the commas
became optional...and before I knew it, I'd basically just reinvented the
same syntax for writing commands as everyone had already been using for a
multitude of decade prior. What started out as the example above ended up
...
1. Many of the flags passed are verbose. Calling .NET objects can be verbose.
   ...
   There is no question that Powershell is a more powerful REPL than Bash but
   it definitely slides towards the "write-once read-many" end of the spectrum.
