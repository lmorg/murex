...
func cmdFidList(p *proc.Process) error {
	...
	p.Stdout.Writeln([]byte(fmt.Sprintf("%7s  %7s  %-12s  %-3s  %-10s  %-10s  %-10s  %s", "FID", "Parent", "State", "BG", "Out Pipe", "Err Pipe", "Command", "Parameters")))
	...
	for i := range procs {
		...
		s := fmt.Sprintf("%7d  %7d  %-12s  %-3s  %-10s  %-10s  %-10s  %s",
			procs[i].Id,
			procs[i].Parent.Id,
			procs[i].State,
