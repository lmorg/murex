...
const eventType = "onCommandError"

func init() {
	event := newCommandError()
...
type commandErrorEvents struct {
	events map[string]commandErrorEvent
	mutex  sync.Mutex
}

func newCommandError() *commandErrorEvents {
	cce := new(commandErrorEvents)
	cce.events = make(map[string]commandErrorEvent)
	return cce
}

// Add a command to the onCommandCompleteionEvents
func (evt *commandErrorEvents) Add(name, command string, block []rune, fileRef *ref.File) error {
	...
	evt.events[name] = commandErrorEvent{
		Command: command,
		Block:   block,
		FileRef: fileRef,
	}
...
func (evt *commandErrorEvents) Remove(name string) error {
	evt.mutex.Lock()
...
func (evt *commandErrorEvents) callback(command string, p *lang.Process) {
	if command == "exec" || command == "fexec" {
		return
	}
...
func (evt *commandErrorEvents) exists(command string) bool {
	evt.mutex.Lock()
...
func (cce *commandErrorEvent) execEvent(name string, parameters []string, p *lang.Process) {
	var err error
...
func (evt *commandErrorEvents) Dump() interface{} {
	dump := newCommandError()
