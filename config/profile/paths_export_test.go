...
func TestProfilePaths(t *testing.T) {
	...
	var path string
	home := home.MyDir
	temp := t.TempDir() + consts.PathSlash
	...
	path = profile.PreloadPathTestable()
	...
	path = profile.ModulePathTestable()
	if !strings.HasPrefix(path, home) {
	...
	path = profile.ProfilePathTestable()
	if !strings.HasPrefix(path, home) {
...
func TestProfileAndCustomPaths(t *testing.T) {
	...
	path := t.TempDir() + consts.PathSlash
	...
	if err != nil {
		t.Fatalf("Unable to stat '%s': %s", filename, err.Error())
	}
	if !fi.IsDir() {
