...
func Execute(flags int) {
	...
	if flags&F_PRELOAD != 0 {
		if err := profile(preloadFileName, PreloadPath()); err != nil {
			tty.Stderr.WriteString("There were problems loading profile `" + PreloadPath() + "`:" + utils.NewLineString)
	...
	if flags&F_MODULES != 0 {
		if err := modules(ModulePath()); err != nil {
			tty.Stderr.WriteString("There were problems loading modules `" + ModulePath() + "`:" + utils.NewLineString)
			tty.Stderr.WriteString(err.Error() + utils.NewLineString)
		}
	}
	...
	if flags&F_PROFILE != 0 {
		if err := profile(profileFileName, ProfilePath()); err != nil {
			tty.Stderr.WriteString("There were problems loading profile `" + ProfilePath() + "`:" + utils.NewLineString)
			tty.Stderr.WriteString(err.Error() + utils.NewLineString)
		}
	}
...
func profile(name, path string) error {
	...
	tty.Stderr.WriteString("Loading profile `" + name + "`" + utils.NewLineString)
